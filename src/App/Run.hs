    ...
    runSqlConn $ do
      ...
      case mapMaybe unValue result of
        comic : _ -> do
          ...
          insertComic comic title folder
        _ -> do
          -- SELECT MAX(comic)+1
          --   FROM comics;
          result <- select $ do
          ...
          case mapMaybe unValue result of
            comic : _ -> do
              insertComic comic title folder
            _ ->
              insertComic (Comic 1) title folder
  where
    insertComic comic title folder =
      insert_ $
        Comics
...
runRemoveComic :: Int -> RIO App ()
runRemoveComic comic = do
  dbFile <- asks (.dbFile)
  bracket (createSqlBackend dbFile) (liftIO . close') $
    runSqlConn $
      delete $ do
