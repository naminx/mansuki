...
const blobURLtoDataURL = (url) => fetch(url).then((response) => response.blob())
  .then((blob) => new Promise((resolve, reject) => { const reader = new FileReader();
...
for (var n = 1; n <= num_pages; ++n) {
  console.log("" + n + "/" + num_pages);
  do { await sleep(100); } while (document.evaluate('count(//div[@id="content-p' + n + '"]/div/div/img)',
    document, null, XPathResult.NUMBER_TYPE, null).numberValue != 3);
  const imgs = xpaths('//div[@id="content-p' + n + '"]/div/div/img');
  await blobURLtoDataURL(imgs[0].src).then(function (dataUrl) { const img =
    document.createElement("img"); img.src = dataUrl; header.append(img); });
  await blobURLtoDataURL(imgs[1].src).then(function (dataUrl) { const img =
    document.createElement("img"); img.src = dataUrl; header.append(img); });
  await blobURLtoDataURL(imgs[2].src).then(function (dataUrl) { const img =
    document.createElement("img"); img.src = dataUrl; header.append(img); });
}
console.log("done");
